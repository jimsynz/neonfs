defmodule NeonFS.S3.HealthPlug do
  @moduledoc """
  Plug that wraps S3Server.Plug with health endpoint and degraded mode handling.
  ...
  adds `X-NeonFS-Status` headers and delegates to the inner S3 plug.
  """
  ...
  def init(opts) do
    inner_opts = S3Server.Plug.init(opts)
  ...
  def call(conn, opts) do
    ...
    case status.status do
      :ok ->
        S3Server.Plug.call(conn, opts.inner)
  ...
  defp encode_health(status) do
    # Manual JSON encoding to avoid adding Jason as a runtime dependency
