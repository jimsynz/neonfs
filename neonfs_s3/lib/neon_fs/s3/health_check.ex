defmodule NeonFS.S3.HealthCheck do
  @moduledoc """
  S3-specific health checks registered with the universal framework.
  """
  ...
  @doc """
  Registers S3 health checks with the universal framework.
  """
  ...
  @doc """
  Returns the current cluster status for the health endpoint.

  Uses the default core node discovery function unless overridden
  via `:neonfs_s3, :core_nodes_fn` application env.
  """
  @spec cluster_status() :: %{
  ...
  @doc """
  Returns the current cluster status using the given core node lookup function.
  """
  ...
  defp core_nodes_fn do
    case Application.get_env(:neonfs_s3, :core_nodes_fn) do
  ...
  defp check_registrar do
    case Process.whereis(NeonFS.Client.Registrar.S3) do
      nil ->
