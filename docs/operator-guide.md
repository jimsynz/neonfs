...
| `RELEASE_COOKIE_PATH` | `/var/lib/neonfs/.erlang.cookie` | File holding the Erlang distribution cookie. Generated by the daemon and managed by the cluster join flow — do not create, edit, or copy it by hand. Node authentication is certificate-based (TLS distribution), not cookie-based. |
...
- [ ] The daemon started cleanly and generated its TLS material under `$NEONFS_TLS_DIR` (`local-ca.crt`, `node-local.crt`) and its cookie file at `/var/lib/neonfs/.erlang.cookie` — both are created automatically on first boot; nothing to generate by hand.
...
Interface packages (`neonfs-fuse`, `neonfs-nfs`, `neonfs-s3`, `neonfs-webdav`, `neonfs-docker`) don't take part in Ra consensus — they register automatically via `NeonFS.Client.Registrar` as soon as they connect to a core node. The prerequisites are:

- The cluster's distribution credentials present under `/var/lib/neonfs`
  (the cluster-signed TLS certificates in `$NEONFS_TLS_DIR` and the
  managed cookie file). When the interface runs alongside a core or
  omnibus daemon — the standard split-services layout — these are
  already in place; there is nothing to generate or copy by hand.
...
**Distribution authentication failure.** Nodes start but refuse to connect — TLS handshake errors in the logs, or `Connection attempt from node ... rejected`.

Node authentication is certificate-based. Check the node's TLS material under `$NEONFS_TLS_DIR`: a joined node should hold the cluster CA (`ca.crt`) and a cluster-signed certificate (`node.crt`) alongside its first-boot local CA pair. Verify certificate validity dates against `neonfs cluster ca info` — an expired CA or node certificate is the most common cause (see the [CA rotation runbook](runbooks/CA-Rotation.md)). A node that never completed its join only has its local certificate and cannot authenticate; redeem a fresh invite. If certificates check out, confirm the managed cookie file (`/var/lib/neonfs/.erlang.cookie`) exists with mode `0600` and owner `neonfs:neonfs` — it is generated by the daemon and updated by the join flow, and should never be edited or copied by hand.
