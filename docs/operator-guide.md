...
Inspect which nodes are up; if a majority is reachable, Ra will elect a new leader within seconds. If a minority is reachable, decide between waiting for the minority to come back or (risky, data-loss-capable) a manual reset. See the operational runbooks in the wiki.
