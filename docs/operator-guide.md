...
- [ ] The daemon started cleanly and generated its TLS material under `$NEONFS_TLS_DIR` (`local-ca.crt`, `node-local.crt`) — created automatically on first boot; nothing to generate by hand. (The Erlang cookie is the constant `neonfs` everywhere; it is not a secret and needs no setup.)
...
- The cluster's distribution credentials present under `/var/lib/neonfs`
  (the cluster-signed TLS certificates in `$NEONFS_TLS_DIR`). When the
  interface runs alongside a core or omnibus daemon — the standard
  split-services layout — these are already in place; there is nothing
  to generate or copy by hand.
