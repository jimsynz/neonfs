defmodule NeonFS.Integration.FileWriteContentionBenchTest do
  ...
  test "concurrent disjoint writers: file volume vs block volume", %{cluster: cluster} do
    file_vol = create_volume(cluster, "contend-file", %{})

    block_vol =
      create_volume(cluster, "contend-block", %{"type" => "block", "max_size" => @file_size})

    file = measure(cluster, file_vol, "/contended.bin")
    block = measure(cluster, block_vol, nil)
    ...
    IO.puts(format(:"block volume (span-scoped compare, #1910)", block))
    IO.puts("  ---")
