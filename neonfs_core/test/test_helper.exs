...
# pool; start a suite-global instance so tests that drive FileIndex writes
# can commit. Workers carry no per-test state — they apply with the writer
# opts each FileIndex call passes them.
{PartitionSupervisor, volume_committer_opts} = NeonFS.Core.VolumeCommitter.pool_spec()
{:ok, _} = PartitionSupervisor.start_link(volume_committer_opts)
