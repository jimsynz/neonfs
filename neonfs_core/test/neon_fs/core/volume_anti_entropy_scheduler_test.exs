defmodule NeonFS.Core.VolumeAntiEntropySchedulerTest do
  # Stubs share `:persistent_term` keyed by `:vae_test_tid` so the
  # tests must be sequential.
  use ExUnit.Case, async: false

  alias NeonFS.Core.{Job, VolumeAntiEntropyScheduler}
  ...
  defmodule Store do
    ...
    @spec init() :: :ets.tid()
    def init, do: :ets.new(:vae_test_store, [:set, :public])
  ...
  defmodule Registry do
    ...
    @spec list() :: [map()]
    def list do
      tid = :persistent_term.get(:vae_test_tid)
  ...
  defp segment(volume_id, scrub_schedule) do
    ...
    %{base | schedules: Map.put(base.schedules, :anti_entropy, scrub_schedule)}
  end
  ...
  defp setup_scheduler(opts) do
    ...
    :persistent_term.put(:vae_test_tid, tid)

    on_exit(fn ->
      try do
        :persistent_term.erase(:vae_test_tid)
      rescue
        ArgumentError -> :ok
      end
    end)
    ...
    {:ok, pid} =
      VolumeAntiEntropyScheduler.start_link(
        name: name,
  ...
  describe "scheduled tick" do
    test "dispatches VolumeAntiEntropy per due volume and updates last_run" do
      attach_telemetry([[:neonfs, :volume_anti_entropy_scheduler, :triggered]])
    ...
    test "skips a volume whose interval hasn't elapsed" do
      ...
      refute_receive {[:neonfs, :volume_anti_entropy_scheduler, :triggered], _,
                      %{volume_id: "vol-not-due"}},
                     500
    end

    test "does not double-dispatch when a VolumeAntiEntropy job for the same volume is running" do
      attach_telemetry([
      ...
      assert_receive {[:neonfs, :volume_anti_entropy_scheduler, :skipped], _,
  ...
  describe "trigger_now/2" do
    test "dispatches an immediate VolumeAntiEntropy job for the named volume" do
      attach_telemetry([[:neonfs, :volume_anti_entropy_scheduler, :triggered]])
      ...
      assert {:ok, _job} = VolumeAntiEntropyScheduler.trigger_now("vol-manual", ctx.name)
