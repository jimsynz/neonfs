defmodule NeonFS.Core.DriveManager do
  ...
  `Cluster.Init.init_cluster/1` and `Cluster.Join.join/2` after Ra is
  available, so the bootstrap layer reflects the local drives before
  the system volume tries to provision per-volume metadata.

  Best-effort per drive — a failed Ra command logs a warning and the
  next drive is attempted. Returns `:ok` either way.
  """
