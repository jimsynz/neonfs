defmodule NeonFS.Core.WriteOperation do
  ...
  @doc """
  Zero-fills `length` bytes at `offset` of an existing file in a single
  metadata commit.

  Writing the same range through `write_file_at_by_id/5` costs one commit
  per call, so a caller bounding its memory has to batch and pays a commit
  per batch — discarding a gigabyte becomes hundreds of commits for a
  result that is one deduped zero chunk. This walks the chunk list
  instead: a chunk the range covers entirely is *replaced* by a stored
  zero chunk of its own size rather than rewritten, and only the partial
  chunks at either end are read-modify-written. One zero chunk is stored
  per distinct covered-chunk size, so the peak working set is one chunk
  regardless of `length`.

  The range must lie within the file — this never grows it, and unlike an
  offset write there is no data to place beyond the end.

  Replicated volumes only; erasure-coded volumes return
  `{:error, :zero_fill_not_supported_for_erasure}`, since a stripe cannot
  be rebuilt from a substituted chunk without re-encoding it.
  """
  @spec write_zeroes_at_by_id(
          binary(),
          binary(),
          non_neg_integer(),
          non_neg_integer(),
  ...
  def write_zeroes_at_by_id(volume_id, file_id, offset, length, opts \\ []) do
    Logger.metadata(component: :write, volume_id: volume_id, file_id: file_id)
    ...
    result =
      with {:ok, volume} <- get_volume(volume_id),
           {:ok, file_meta} <- get_file_by_id(volume_id, file_id),
           :ok <- authorise_file_write(uid, gids, volume_id, file_meta.path),
           :ok <- check_lock(volume_id, file_meta.path, client_ref, {offset, length}, opts) do
        do_write_zeroes_at(volume, file_meta, offset, length, write_id, opts)
      end
  ...
  defp do_write_at(volume, file_meta, offset, data, write_id, opts, attempt \\ 1) do
    ...
    with :ok <- check_capacity(volume, max(0, write_end - file_meta.size)),
      ...
      # then failed and correctly took it away: the reference this write was
      # about to adopt is gone by the time it adopts it. That is a lost race
      # like a stale CAS, not a fault — the data is simply not there to share
      # any more, and re-reading rebuilds the write against what is.
      #
      # It is routed through the same bounded retry, which is also what tells
      # it from a chunk that is genuinely missing: the latter recurs against
      # every fresh read until the budget is spent and the error is returned
      # unchanged.
      {:error, {:add_write_ref_failed, :not_found}} ->
        retry_write_at(
          volume,
          file_meta,
          offset,
          ...
          opts,
          attempt,
          {:add_write_ref_failed, :not_found}
        )
  ...
  defp do_write_zeroes_at(
         %{durability: %{type: :erasure}},
         _file_meta,
         _offset,
         _length,
         _write_id,
    ...
    {:error, :zero_fill_not_supported_for_erasure}
  end
  ...
  defp do_write_zeroes_at(volume, file_meta, offset, length, write_id, opts) do
    ...
    with :ok <- validate_zero_range(file_meta, write_end),
         ...
         write_ctx = %{write_id: write_id, enc_ctx: enc_ctx},
         {:ok, replacements} <-
           zero_replacement_chunks(affected, offset, write_end, volume, write_ctx, opts) do
      commit_zeroed_range(volume, file_meta, prefix, replacements, suffix, write_id)
    else
      {:error, _reason} = error ->
        abort_chunks(write_id)
        error
  ...
  # Nothing overlaps the range — an unwritten region already reads as
  # zeroes, so there is no commit to make.
  defp commit_zeroed_range(_volume, file_meta, _prefix, [], _suffix, _write_id) do
    {:ok, file_meta}
  end

  defp commit_zeroed_range(volume, file_meta, prefix, replacements, suffix, write_id) do
    prefix_hashes = Enum.map(prefix, fn {hash, _start, _end} -> hash end)
    suffix_hashes = Enum.map(suffix, fn {hash, _start, _end} -> hash end)
    all_hashes = prefix_hashes ++ Enum.map(replacements, & &1.hash) ++ suffix_hashes

    # One zero chunk can land at many positions, so the same hash repeats in
    # the file's chunk list. Write refs and volume stats count chunks, not
    # positions — passing the repeats would ref and account each one twice.
    stored = Enum.uniq_by(replacements, & &1.hash)

    case update_file_and_commit(file_meta.volume_id, file_meta.id, write_id, stored,
           chunks: all_hashes,
           size: file_meta.size
         ) do
      {:ok, _updated_meta} = ok ->
        safe_update_volume_stats(volume.id, 0, stored)
        ok
  ...
  defp validate_zero_range(%{size: size}, write_end) when write_end <= size, do: :ok

  defp validate_zero_range(%{size: size}, write_end),
    do: {:error, {:zero_range_beyond_eof, write_end, size}}

  # Walks the covered chunks in order, carrying the zero chunks stored so
  # far keyed by size so a range spanning a thousand equal chunks stores one.
  defp zero_replacement_chunks(affected, offset, write_end, volume, write_ctx, opts) do
    affected
    |> Enum.reduce_while({:ok, [], %{}}, fn chunk, {:ok, acc, zeros} ->
      case replacement_chunk(chunk, offset, write_end, zeros, volume, write_ctx, opts) do
        {:ok, chunk_info, zeros} -> {:cont, {:ok, [chunk_info | acc], zeros}}
        {:error, _reason} = error -> {:halt, error}
      end
    end)
    |> case do
      {:ok, acc, _zeros} -> {:ok, Enum.reverse(acc)}
      {:error, _reason} = error -> error
    end
  end

  # Covered end to end: substituted outright, without reading it back.
  defp replacement_chunk({_hash, start, stop}, offset, write_end, zeros, volume, write_ctx, opts)
       when start >= offset and stop <= write_end do
    zero_chunk_for_size(stop - start, zeros, volume, write_ctx, opts)
  end

  # Only clipped by the range, so its surviving bytes have to be read back.
  defp replacement_chunk({hash, start, stop}, offset, write_end, zeros, volume, write_ctx, opts) do
    with {:ok, chunk_info} <-
           rewrite_partial_chunk(hash, start, stop, offset, write_end, volume, write_ctx, opts) do
      {:ok, chunk_info, zeros}
    end
  end

  defp zero_chunk_for_size(size, zeros, volume, write_ctx, opts) do
    case Map.fetch(zeros, size) do
      {:ok, chunk_info} ->
        {:ok, chunk_info, zeros}

      :error ->
        with {:ok, chunk_info} <-
               store_replacement_chunk(:binary.copy(<<0>>, size), volume, write_ctx, opts) do
          {:ok, chunk_info, Map.put(zeros, size, chunk_info)}
        end
    end
  end

  defp rewrite_partial_chunk(hash, start, stop, offset, write_end, volume, write_ctx, opts) do
    with {:ok, data} <- fetch_chunk_for_write(hash, volume.id) do
      zero_from = max(offset, start) - start
      zero_to = min(write_end, stop) - start

      zeroed =
        binary_part(data, 0, zero_from) <>
          :binary.copy(<<0>>, zero_to - zero_from) <>
          binary_part(data, zero_to, byte_size(data) - zero_to)

      store_replacement_chunk(zeroed, volume, write_ctx, opts)
    end
  end

  # Each replacement is exactly one chunk of the file, so it is stored whole
  # rather than run back through the volume's chunking strategy — which
  # would be free to split it and change the file's chunk boundaries.
  defp store_replacement_chunk(data, volume, write_ctx, opts) do
    case chunk_and_store(data, volume, write_ctx, Keyword.put(opts, :chunk_strategy, {:single})) do
      {:ok, [chunk_info]} -> {:ok, chunk_info}
      {:ok, other} -> {:error, {:unexpected_chunk_count, length(other)}}
      {:error, _reason} = error -> error
    end
  end
  ...
  defp delete_chunk_from_storage(meta) do
    case meta.locations do
      [location | _] ->
        ...
        BlobStore.delete_chunk(meta.hash, drive_id, BlobStore.Codec.opts_for_chunk(meta))
