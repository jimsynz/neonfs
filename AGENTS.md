...
**The Samba build carries no debug info, and that is load-bearing.** `build-vfs-deb.sh` exports `DEB_CFLAGS_APPEND=-g0` and puts `noautodbgsym` in `DEB_BUILD_OPTIONS` because a cold build at Debian's default `-g -O2` does not fit on the runner: it dies partway through the 6600-step build writing `.debug_str` into an object file, and since `actions/cache` only saves on success, the next run is cold again and dies in the same place. Nothing downstream wants that DWARF — the module ships stripped and the `-dbgsym` companions are discarded. Dropping the flags to "get symbols" re-arms a trap that blocks every PR, because `canary` requires this job.
