...
**End-to-end jobs run nightly, never per push, and are not required checks.** `.forgejo/workflows/e2e-nightly.yml` is where a job that boots Kubernetes (or, later, an `smbd`) belongs: on a `schedule` plus `workflow_dispatch`, outside `canary`, so it never competes with the integration shards for the same two amd64 runners. A failure there is a signal to investigate rather than a merge blocker. Its Kubernetes toolchain comes from `resources/scripts/ci-install-k8s-tools`, which pins `kind`, `kubectl`, `helm` and `csi-sanity` and installs them in-job — the CI base images live in another repository, so provisioning cannot happen in the image. The pin is read back with `--print-version` to build the cache key, so the version and the key cannot drift apart; the `neonfs_csi` package job uses the same script and the same key for `csi-sanity`.
