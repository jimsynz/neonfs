...
**Current Status:** the storage engine (replication, erasure coding, tiering, compression, encryption), multi-node clustering (Ra consensus, leaderless quorum metadata, cluster CA, TLS distribution, mTLS data plane), and eight access interfaces (FUSE, NFSv3, S3, WebDAV, CIFS/SMB, Docker volumes, containerd content store, Kubernetes CSI) are shipped. CIFS/SMB via Samba VFS landed most recently — `vfs_neonfs.so` C shim, `neonfs_cifs` Elixir bridge, Debian package, container image, omnibus integration, and an end-to-end test against a real `smbd` that runs nightly — and its tracking epic is still open. See the [issue tracker](https://harton.dev/project-neon/neonfs/issues) for active work.
...
| `neonfs_cifs/` | Samba VFS module backend (ETF over Unix socket). `vfs_neonfs.so` C shim + Elixir bridge, Debian package, container image and omnibus integration all shipped; the end-to-end test against a real `smbd` runs nightly. |
...
**Every workflow's `setup` job resolves its container image through `resources/scripts/ci-toolchain-image`.** The job itself has to be per-workflow — a `container:` key is evaluated before any step in the job that uses it, so an image cannot be computed inside the job it names — but the parsing behind it is shared. The script reads `.tool-versions` and prints `elixir` / `erlang` / `rust` plus the three image tags as `$GITHUB_OUTPUT` lines; a job declares whichever outputs it consumes. Keeping that in one place matters because the drift it prevents is silent: a workflow computing a stale tag names an image that exists and is wrong rather than failing. A missing `.tool-versions` entry now fails the script naming the tool, instead of yielding a tag like `elixir:-erlang-` whose pull error says nothing.

**A native build dependency has to be added in five places, not one.** A crate whose build script needs a system library (`libclang-dev` for anything using bindgen, say) will build in the dev container and fail everywhere else, because the CI base images live in another repository and cannot be changed from here. The list: the package's `ci-matrix.json` `apt` key, *and every other entry whose package compiles it* (`neonfs_omnibus` compiles `neonfs_block`); `containers/Containerfile.base`, which every container build descends from; both architecture jobs in `release.yml`, which build the debs; and `.devcontainer/devcontainer.json`, so a fresh container is not the odd one out. Verify the package name against the actual image rather than from memory — `docker run <ci image> bash -c 'apt-get install -y <pkg> && cargo build'` on the crate takes under a minute and is the difference between one push and three.
...
**End-to-end jobs run nightly, never per push, and are not required checks.** `.forgejo/workflows/e2e-nightly.yml` is where a job that boots Kubernetes or an `smbd` belongs: on a `schedule` plus `workflow_dispatch`, outside `canary`, so it never competes with the integration shards for the same two amd64 runners. A failure there is a signal to investigate rather than a merge blocker. Its Kubernetes toolchain comes from `resources/scripts/ci-install-k8s-tools`, which pins `kind`, `kubectl`, `helm` and `csi-sanity` and installs them in-job — the CI base images live in another repository, so provisioning cannot happen in the image. The pin is read back with `--print-version` to build the cache key, so the version and the key cannot drift apart; the `neonfs_csi` package job uses the same script and the same key for `csi-sanity`.

**The Samba build cache key carries the Samba version.** `build-vfs-deb.sh` refuses to build against a source tree that no longer matches the archive, so a key that does not move with the archive cannot stay usable: `actions/cache` will not overwrite an existing entry, so the first point release after the entry was written poisons it permanently and every later run restores half a gigabyte only to discard it. All three jobs that touch `.samba-build` (the `vfs_neonfs` job, the cache-warm workflow, the nightly `cifs_smbd` job) therefore resolve `apt-cache policy samba` into the key. Bumping the `v5-` prefix by hand fixes today and re-breaks on the next point release.

**The Samba build carries no debug info, and a cold one is tight on disk.** `build-vfs-deb.sh` exports `DEB_CFLAGS_APPEND=-g0` and puts `noautodbgsym` in `DEB_BUILD_OPTIONS`: measured on a cold amd64 build, that halves the peak tree from 1.6G to 828M and stops 27 `-dbgsym` debs being built. Nothing downstream wants the DWARF — the module ships stripped and the dbgsym companions are discarded — so restoring the flags to "get symbols" only buys back the footprint. A cold build at the default `-g -O2` has already exhausted an aarch64 runner mid-compile, and because `actions/cache` only saves on success, a cold cache that cannot finish stays cold and fails the same way next run. `canary` requires this job, so that state blocks every PR. If it recurs even at 828M, the next lever is building fewer of Samba's binary packages, not restoring debug info.

**`-nc` needs `debian/files` deleted, not just the stamps.** The incremental path skips the `dh_clean` that would remove the previous build's manifest, and the script deletes the debs that manifest names — so any entry the new run does not regenerate dangles and `dpkg-genbuildinfo` dies on `cannot fstat`. It bit the switch to `noautodbgsym`, where the dbgsym entries were suddenly never rebuilt. Note that the two build paths fail differently: a cold local build takes the clean path and cannot reproduce this at all, so a fix verified only on a fresh tree is not verified.

**The `smbd` sidecar is the job's own container, not a separate image.** A Samba VFS module must ABI- and symbol-version match the `smbd` that loads it, so the nightly `cifs_smbd` job builds `samba-vfs-neonfs.deb` and then installs it beside `samba` in the very container it was built in. `packaging/verify-smbd-sidecar.sh` makes the match enforced rather than assumed: the package's `Depends: samba (= <exact version>)` means apt refuses a mismatch, the declared version is compared against the archive candidate first so the error names both, and `ldd -r` proves every symbol resolves against the installed Samba — the same failure smbd would hit at load time, made visible. It shares the `v4-vfs_neonfs-samba-deb-<arch>` cache with the `vfs_neonfs` job and the cache-warm workflow rather than building a third Samba tree.
