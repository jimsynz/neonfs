...
**The integration suite is sharded.** `neonfs_integration` is a matrix over `partition: [1, 2, 3, 4]` running `mix test --partitions 4`; its static tools (compiler, credo, dialyzer, formatter, audit) live in a separate `neonfs_integration_checks` job so they run once rather than per shard. Two things must stay in step when changing the shard count: the literal `partition` list and the `--partitions N` argument. Mix distributes test *files* round-robin over their sorted names, so a shard's contents shift whenever a file is added or renamed — balance is approximate by design, and one shard running slower than its siblings is expected rather than a fault. Only the sharded job needs the privileged container, the loop devices and the registry sidecar.
